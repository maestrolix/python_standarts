...
from ssd_libs.app.errors import AppError, ErrorsList
...
class App(falcon.App):
    ...
    def register(self, controller, url=None):
        if url is None:
            url = '/' + utils.camel_case_to_dash(controller.__class__.__name__)

        for suffix in utils.get_suffixes(controller):
            self.add_method(f'{url}/{suffix}', controller, suffix=suffix)

    def add_method(self, url, controller, suffix=None):
        self.add_route(f'{self.url_prefix}{url}', controller, suffix=suffix)
