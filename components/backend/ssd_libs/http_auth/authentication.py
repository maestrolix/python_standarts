...
class Authenticator:
    ...
    def set_strategies(
        self,
        strategies: Union[interfaces.AuthStrategy,
                          Sequence[interfaces.AuthStrategy]],
    ):
        self.strategies = (
            tuple(strategies)
            if isinstance(strategies, Sequence) else (strategies, )
        )
    ...
    def _try_to_get_client(
        ...
        if client:
            ...
            self.logger.debug(
                'No client received by strategy [%s]', strategy_name
            )
        return client
    ...
    def auth(self, request: falcon.Request, resource_name: str = ""):
        ...
        if not client:
            raise errors.AuthenticationIsNotAvailable(
