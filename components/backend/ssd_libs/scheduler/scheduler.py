...
class Scheduler:
    ...
    def __init__(self, *tasks: Task):
        ...
        # TODO возможно, это проблемное место.
        # Так как, ThreadPoolExecutor, создает потоки на каждую выполняемую задачу.
        # Из-за этого могут быть проблемы как с подключением с БД (слишком большое количество сессий, один поток - одна сессия)
        # Так и проблемы с RabbitMQ, так как он работает, по тому же принципу
        #
        # Одно из решение ограничить количество воркеров, ядрами процессора,
        # Но данный вариант нужно протестировать. Но исчезнет проблема с превышением допустимого кол-ва сессий.
